...
Public Class rp_job_process_cost_cpp
    ...
    Private Sub cmdSearch_Click(sender As Object, e As EventArgs) Handles cmdSearch.Click
        ...
        dt_main = config.GetDataTable("SELECT  strftime('%Y-%m' , event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes') as event_date , jobs_name , jobs_op_name, jobs_op_tools_name , SUM( stock_qty * stock_price) as cost, " &
                                      ...
                                      "AND productions_month = CAST(strftime('%m' , tb1.event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes')  AS INTEGER)" &
                                       "AND productions_year = CAST(strftime('%Y' , tb1.event_date  , '-" & cut_stock_day.ToString & " days')  AS INTEGER)" &
                                      ...
                                      "AND tb1.jobs_op_tools_id IN (" & str_op & ") " &
                                      str_group & str &
        ...
        If cbChartType.SelectedItem = "Candle" Then
            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_job_process_cost_cpp.rdlc"
        ElseIf cbChartType.SelectedItem = "Pie" Then

            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_job_process_cost_cpp_PIE.rdlc"
        Else
            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_job_process_cost_cpp_Line.rdlc"
        End If
