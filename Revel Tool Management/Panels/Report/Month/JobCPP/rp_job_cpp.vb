...
Public Class rp_job_cpp
    ...
    Private Sub cmdSearch_Click(sender As Object, e As EventArgs) Handles cmdSearch.Click
        ...
        dt_main = config.GetDataTable("SELECT  strftime('%Y-%m' , event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes') as event_date , jobs_name , SUM( stock_qty * stock_price) as cost ,  " &
                                      " IFNULL((" &
                                      "SELECT productions_qty FROM tb_productions WHERE productions_jobs_id = tb1.jobs_id  " &
                                      "AND productions_month = CAST(strftime('%m' , tb1.event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes')  AS INTEGER)" &
                                       "AND productions_year = CAST(strftime('%Y' , tb1.event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes')  AS INTEGER)" &
                                      ...
                                      str_group & str &
                                      "GROUP BY strftime('%Y-%m' , event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes'), jobs_name " &
                                      "ORDER BY strftime('%Y-%m' , event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes'), jobs_name DESC ")


        Dim sum_max As Double = 0

        For i As Int16 = 0 To dt_main.Rows.Count - 1
            sum_max += Convert.ToDouble(dt_main.Rows(i).Item(2).ToString())
        Next

        Dim cost_max As New ReportParameter("cost_max", sum_max)

        If cbChartType.SelectedItem = "Candle" Then
            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_job_cpp.rdlc"
        ElseIf cbChartType.SelectedItem = "Pie" Then

            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_job_cpp_PIE.rdlc"
        Else
            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_job_cpp_Line.rdlc"
        End If
        ...
        If cbChartType.SelectedItem = "Pie" Then
            dt_sum = config.GetDataTable("SELECT  strftime('%Y-%m' , event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes') as event_date , jobs_name , SUM( SUM( stock_qty * stock_price) / IFNULL((" &
                                      "SELECT productions_qty FROM tb_productions WHERE productions_jobs_id = tb1.jobs_id  " &
                                      "AND productions_month = CAST(strftime('%m' , tb1.event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes')  AS INTEGER) " &
                                       "AND productions_year = CAST(strftime('%Y' , tb1.event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes')  AS INTEGER) - 543 " &
                                      "),'1') ) as cost " &
                                      "FROM tb_stock_log " &
                                      "LEFT JOIN tb_how2buy ON tb1.how2buy_id = tb_how2buy.how2buy_id " &
                                      "LEFT JOIN tb_products ON tb_how2buy.products_id = tb_products.products_id  " &
