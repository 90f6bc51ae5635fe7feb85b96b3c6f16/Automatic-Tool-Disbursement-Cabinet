...
Public Class rp_stock_issue_product_cost
    ...
    Private Sub cmdSearch_Click(sender As Object, e As EventArgs) Handles cmdSearch.Click
        ...
        dt_main = config.GetDataTable("SELECT  strftime('%Y-%m' , event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes') as event_date, products_code , SUM( stock_qty * stock_price) as cost " &
                                      "FROM tb_stock_log " &
                                      ...
                                      "AND tb_how2buy.products_id IN (" & str_jobs & ") " & str &
                                      "GROUP BY strftime('%Y-%m' , event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes'), products_code " &
                                      "ORDER BY strftime('%Y-%m' , event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes'), products_code DESC ")
        ...
        If cbChartType.SelectedItem = "Candle" Then
            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_stock_issue_product_cost.rdlc"
        ElseIf cbChartType.SelectedItem = "Pie" Then



            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_stock_issue_product_cost_PIE.rdlc"
        Else
            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_stock_issue_product_cost_Line.rdlc"
        End If
    ...
    Private Sub cbJobs_SelectedIndexChanged(sender As Object, e As EventArgs)
