...
Public Class rp_job_operation_cost
    ...
    Private Sub cmdSearch_Click(sender As Object, e As EventArgs) Handles cmdSearch.Click
        ...
        For Each Item In lbOperation.SelectedItems
        ...
        dt_main = config.GetDataTable("SELECT  strftime('%Y-%m' , event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes') as event_date , jobs_name , jobs_op_name , SUM( stock_qty * stock_price) as cost " &
                                      "FROM tb_stock_log " &
                                      ...
                                      "AND tb_stock_log.jobs_op_id IN (" & str_op & ") " &
                                      str_group & str &
                                      "GROUP BY strftime('%Y-%m' , event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes'), jobs_name, jobs_op_name " &
                                      "ORDER BY strftime('%Y-%m' , event_date  , '-" & cut_stock_day.ToString & " days' , '-" & cut_stock_hour.ToString & " hours','-" & cut_stock_min.ToString & " minutes'), jobs_name, jobs_op_name DESC ")


        Dim sum_max As Double = 0

        For i As Int16 = 0 To dt_main.Rows.Count - 1
            sum_max += Convert.ToDouble(dt_main.Rows(i).Item(3).ToString())
        Next

        Dim cost_max As New ReportParameter("cost_max", sum_max)

        If cbChartType.SelectedItem = "Candle" Then
            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_job_operation_cost.rdlc"
        ElseIf cbChartType.SelectedItem = "Pie" Then

            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_job_operation_cost_PIE.rdlc"
        Else
            Me.rvView.LocalReport.ReportEmbeddedResource = "Revel_Tool_Management.rp_job_operation_cost_Line.rdlc"
        End If
    ...
    Sub getJobs()
        ...
        Dim dt_op As DataTable = config.GetDataTable("SELECT * FROM tb_jobs_op WHERE jobs_id = '" & cbJobs.SelectedValue.ToString & "'")


        lbOperation.DataSource = dt_op
        lbOperation.DisplayMember = "jobs_op_name"
        lbOperation.ValueMember = "jobs_op_id"
    ...
    Private Sub cbJobs_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbJobs.SelectedIndexChanged
        ...
        If cbJobs.SelectedIndex > -1 Then
            Dim dt As DataTable = config.GetDataTable("SELECT * FROM tb_jobs_op WHERE jobs_id = '" & cbJobs.SelectedValue.ToString & "'")


            lbOperation.DataSource = dt
            lbOperation.DisplayMember = "jobs_op_name"
            lbOperation.ValueMember = "jobs_op_id"

            lbOperation.SelectedItems.Clear()
        End If
    ...
    Private Sub cmdSelectAll_Click(sender As Object, e As EventArgs) Handles cmdSelectAll.Click
        For i As Int32 = 0 To lbOperation.Items.Count - 1
            lbOperation.SetSelected(i, True)
        Next
    End Sub

    Private Sub cmdUnselect_Click(sender As Object, e As EventArgs) Handles cmdUnselect.Click
        For i As Int32 = 0 To lbOperation.Items.Count - 1
            lbOperation.SetSelected(i, False)
        Next
    End Sub
End Class